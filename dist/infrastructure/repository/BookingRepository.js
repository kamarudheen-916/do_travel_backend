...
class BookingRepository {
    async checkRoomAvailability(roomId, checkInDate, checkOutDate) {
        try {
            const roomData = await propertyRoom_1.default.findById(roomId);
    ...
    async uploadBookingData(BookingData) {
        try {
            const res = await BookingModel_1.default.insertMany([BookingData]);
            const roomData = await propertyRoom_1.default.findOne({ _id: BookingData.roomId });
            if (roomData) {
    ...
    async fetchAllBookings(UserId) {
        try {
            if (UserId) {
                const doc = await BookingModel_1.default.find({ bookingUserId: UserId });
                if (doc) {
                    console.log(doc);
    ...
    async cancelBookings(BookingId) {
        try {
            const res = await BookingModel_1.default.updateOne({ _id: BookingId }, { $set: { bookingStatus: 'cancelled' } });
