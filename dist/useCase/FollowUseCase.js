...
class FollowUseCase {
    ...
    async fetchAllFollowData(userId) {
        try {
            let followingData = [], followerData = [];
            const Res = await this.IfollowRepo.getAllFollwers(userId);
    ...
    async fetchFollowerOriginalData(userId, isFollowing) {
        try {
            const Res = await this.IfollowRepo.getAllFollwers(userId);
            // console.log('========>',Res); 
            let followerData;
